...
class tx_gtnaddresses_pi1 extends tslib_pibase {
	...
	function main($content, $conf) {
		...
		if ($action=="DRUPAL_IMPORT"){
			...
			*/
				...
				foreach ($filters as $filter=>$val)
					...
					switch($filter) {
						...
						case 'ort': $where .= " AND (a.praxis_ort LIKE '%".$val."%' OR a.praxis2_ort LIKE '%".$val."%')";							
							break;
						case 'name': $where .= " AND (a.nachname LIKE '%".$val."%' OR a.vorname LIKE '%".$val."%')";
							break;							
			...
			$fields="a.*";
			$i=1;
			if ($result=$this->get_address_records($fields,$where,"group by a.uid order by a.promote DESC,a.sticky DESC, a.nachname")){
				$imgTSConfig = array();
				...
				while($rs=mysql_fetch_array($result)){
					...
					$markerArray["###email###"]=$rs["email"];
 					$markerArray["###image###"]=$rs["dateien"];
			...
			if ($this->piVars["deldatei"]!="" && intval($this->piVars["uid"])>0){
				$this->update_datei_in_db("tx_gtnaddresses_data",$this->clean_param($this->piVars["deldatei"],"PARAM_FILE"),intval($this->piVars["uid"]));
			}
					...
					*/
					...
					foreach($tca["columns"] as $k=>$tcapart){
						if($tcapart["config"]["eval"]=="date"){
							...
							$newfilen=$this->hidepath($this->storeImage($k,$this->bildpfad,$rs[$k],$tcapart["config"]["max_size"]));
	...
	function storeImage($datei,$bildpfad,$dbwert,$max_size)	{
		//echo $datei." -- <br> ".$bildpfad;
	...
	function person_detail($tmpl, $id) {
		...
		// revision_titel
		$markerArray["###FIELD_REVISION_TITEL###"] = $person['revision_titel'];
		// titel
		...
		if ($person['fe_uid']>0) {
			...
			$markerArray["###FE_USER_REGDATE###"] = date($this->conf["format_date"], $feuser['crdate']);
			$subpartsArray["###FE_USER###"] = $this->cObj->substituteMarkerArrayCached($temp_tmpl, $markerArray);
		}
		else $subpartsArray["###FE_USER###"] = "";
		// bereiche
		$ber = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows("*", "tx_gtnaddresses_data_cat_mm mm LEFT JOIN tx_gtnaddresses_cat c ON mm.uid_foreign=c.uid", "mm.uid_local=".$person['uid']);
		if (count($ber)>0) {
		...
		$fileimages	= explode(",",$person['images']);
		if ($fileimages[0])
		...
		else $subpartsArray["###ATTACHMENTS###"] = "";
...
//		print_r($person);
	...
	function get_select_list($tmpl, $arr, $curr_val, $label="", $sort=0) {	
