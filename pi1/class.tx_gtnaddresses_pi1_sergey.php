...
//				echo $where;			
			...
			$fields="a.*";
			$i=1;
			if ($result=$this->get_address_records($fields,$add_inner,$where,"group by a.uid order by a.promote DESC,a.sticky DESC, a.nachname")){
				$imgTSConfig = array();
...
//					$markerArray["###image###"]=$rs["images"];
					$fileimages	= explode(",",$rs["images"]);
			...
			if ($this->piVars["deldatei"]!="" && intval($this->piVars["uid"])>0){
				$this->update_datei_in_db("tx_gtnaddresses_data",$this->piVars["deldatei"],intval($this->piVars["uid"]));
			}
